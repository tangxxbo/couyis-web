...
class Product extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        ProductService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width:60, render: (text, record, index) => `${index + 1}`},
		                    { title: '编号', width: 150, dataIndex: 'code', key: 'code', sorter: true,...this.getSearchProps("code")},
                            { title: '名称', width: 200, dataIndex: 'name', key: 'name',sorter: true,...this.getSearchProps("name") },
                            { title: '单位', width: 100, dataIndex: 'unitCode', key: 'unitCode'},
                            { title: '建议价格', dataIndex: 'suggestedPrice', key: 'suggestedPrice', width: 100, },
                            { title: '产品类别', dataIndex: 'productCategoryName', key: 'productCategoryName', width: 100, },
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                        <Authority authorityId='PRODUCT_EDIT'>
                        ...
                        <Authority authorityId='PRODUCT_DELETE'>
				...
				<Table
                    ...
                    rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加产品' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                     <ProductAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
                <Modal title='修改产品' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <ProductEdit ref={this.onEditRef} product={this.state.product} handEditOKFinish ={this.handEditOKFinish}/>
                </Modal>
			</div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await ProductService.getToken()
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            product:await ProductService.getProduct(this.state.selectedRowKeys[0])
        })
    }
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await ProductService.delete(selectedRowKeys)
