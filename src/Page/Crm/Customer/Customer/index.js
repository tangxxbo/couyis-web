...
class Customer extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        CustomerService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, fixed: 'left',},
		                    { title: '编号', width: 120, dataIndex: 'code', key: 'code', sorter: true,...this.getSearchProps("code") ,fixed: 'left',},
                            ...
                            { title: '企业性质', dataIndex: 'ep', key: 'ep', width: 100, },
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                        <Authority authorityId='CUSTOMER_EDIT'>
                        ...
                        <Authority authorityId='CUSTOMER_DELETE'>
				...
				<Table
                    ...
                    rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加客户信息' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                     <CustomerAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
                <Modal title='修改客户信息' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <CustomerEdit ref={this.onEditRef} customer={this.state.customer} handEditOKFinish ={this.handEditOKFinish}/>
                </Modal>
			</div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await CustomerService.getToken()
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            customer:await CustomerService.getCustomer(this.state.selectedRowKeys[0])
        })
    }
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await CustomerService.delete(selectedRowKeys)
