...
import CaresAdd from './Add'
import CaresEdit from './Edit'
const {confirm} = Modal
class Cares extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        CaresService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 50, render: (text, record, index) => `${index + 1}`,},
		                    { title: '主题', width: 200, dataIndex: 'subject', key: 'subject', sorter: true,...this.getSearchProps("subject") },
		                    { title: '时间', width: 120, dataIndex: 'caresTime', key: 'caresTime',sorter: true },
                            { title: '联系人', dataIndex: 'contactsName', key: 'contactsName', sorter: true,...this.getSearchProps("contactsName"), width: 120},
                            { title: '客户', dataIndex: 'customerName', key: 'customerName', sorter: true,...this.getSearchProps("customerName"), width: 200},
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                        <Authority authorityId='CARES_EDIT'>
                        ...
                        <Authority authorityId='CARES_DELETE'>
				...
				<Table
                    ...
                    rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加客户关怀' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                    <CaresAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish}  />
                </Modal>
                <Modal title='修改客户关怀' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <CaresEdit ref={this.onEditRef} cares={this.state.cares} handEditOKFinish ={this.handEditOKFinish} />
                </Modal>                
			</div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await CaresService.getToken()
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            contocts:await CaresService.getCares(this.state.selectedRowKeys[0])
        })
    }
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await CaresService.delete(selectedRowKeys)
