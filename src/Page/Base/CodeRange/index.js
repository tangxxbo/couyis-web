...
import CodeRangeAdd from './Add'
import CodeRangeEdit from './Edit'
const {confirm} = Modal
class CodeRange extends Component {
    state = {
        dataSource: [],
        pagination:{},
        filters:{},
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        CodeRangeService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, },
                            { title: '编号', width: 100, dataIndex: 'code', key: 'code', sorter: true,...this.getSearchProps("code") },
                            { title: '起始编号', width: 140, dataIndex: 'startCode', key: 'startCode', sorter: true,...this.getSearchProps("startCode") },
                            { title: '截止编号',width: 140,dataIndex: 'stopCode',key: 'stopCode', sorter: true,...this.getSearchProps("stopCode")},
                            { title: '最大号码', width: 150, dataIndex: 'maxCode', key: 'maxCode', sorter: true,...this.getSearchProps("maxCode")},
                            { title: '是否外部给号', width: 160, dataIndex: 'isExt', key: 'isExt',sorter: true,
                            filters :[
                                {text:'否',value:'0'},
                                {text:'是',value:'1'},
                            ],
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='CODE_RANGE_EDIT'>
                            ...
                            <Authority authorityId='CODE_RANGE_DELETE'>
				...
				<Table
					...
					rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加组织' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                    <CodeRangeAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
                <Modal title='修改组织' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <CodeRangeEdit ref={this.onEditRef} codeRange={this.state.codeRange} handEditOKFinish ={this.handEditOKFinish} />
                </Modal>
            </div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:(await CodeRangeService.getToken()),
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            codeRange:await CodeRangeService.getCodeRange(this.state.selectedRowKeys[0]),
        })
    }
    handleEditOK = () =>{       
        this.child.handleOK();
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await CodeRangeService.delete(selectedRowKeys)
