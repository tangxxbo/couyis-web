...
class Unit extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        UnitService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, },
                            ...
                            { title: '名称', width: 140, dataIndex: 'name', key: 'name', sorter: true,...this.getSearchProps("name") },
                            { title: '分类', width: 120, dataIndex: 'categoryName', key: 'categoryName', sorter: true,...this.getSearchProps("categoryName") },
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='UNIT_EDIT'>
                            ...
                            <Authority authorityId='UNIT_DELETE'>
				...
				<Table
					...
					rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加单位' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                    <UnitAdd ref={this.onAddRef} token = {this.state.token}  handAddOKFinish={this.handAddOKFinish}/>
                </Modal>
				...
				okText='提交' cancelText='关闭'>
                <UnitEdit ref={this.onEditRef} unit={this.state.unit} handEditOKFinish ={this.handEditOKFinish}/>
                </Modal>
			</div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:(await UnitService.getToken()),           
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            unit:(await UnitService.getUnit(this.state.selectedRowKeys[0]))
        })
    }
	...
	handleDelete = () => {
		...
		confirm({
			...
			async onOk() {
                await UnitService.delete(selectedRowKeys)
