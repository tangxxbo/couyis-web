...
class Org extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        OrgService.pageList({...pagination, ...filters, ...sorter}).then(res => {
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='ORG_EDIT'>
                            ...
                            <Authority authorityId='ORG_DELETE'>
				...
				okText='提交' cancelText='关闭'>
                    <OrgAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish}/>
                </Modal>
				...
				okText='提交' cancelText='关闭'>
                    <OrgEdit ref={this.onEditRef} org={this.state.org} handEditOKFinish ={this.handEditOKFinish}/>
                </Modal>
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:(await OrgService.getToken()),           
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            org:await OrgService.getOrg(this.state.selectedRowKeys[0]),
        })
    }
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await OrgService.delete(selectedRowKeys)
