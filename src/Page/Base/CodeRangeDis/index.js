...
import CodeRangeDisAdd from './Add'
import CodeRangeDisEdit from './Edit'
const {confirm} = Modal
class CodeRangeDis extends Component {
    state = {
        dataSource: [],
        pagination:{},
        filters:{},
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        CodeRangeDisService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, },
                            { title: '号码段编号', width: 140, dataIndex: 'codeRangeCode', key: 'codeRangeCode', sorter: true,...this.getSearchProps("codeRangeCode")},
                            { title: '赋予项目', width: 140, dataIndex: 'typeUrl', key: 'typeUrl', sorter: true,...this.getSearchProps("typeUrl") },
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='CODERANGE_DIS_ADD'>
                            ...
                            <Authority authorityId='CODERANGE_DIS_DELETE'>
				...
				okText='提交' cancelText='关闭'>
                    <CodeRangeDisAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
				...
				okText='提交' cancelText='关闭'>
                    <CodeRangeDisEdit ref={this.onEditRef} codeRange={this.state.codeRange} handEditOKFinish ={this.handEditOKFinish} />
                </Modal>
            </div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:(await CodeRangeDisService.getToken()),
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            codeRange:await CodeRangeDisService.getCodeRange(this.state.selectedRowKeys[0]),
        })
    }
    handleEditOK = () =>{       
        this.child.handleOK();
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await CodeRangeDisService.delete(selectedRowKeys)
