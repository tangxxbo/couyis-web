...
class Dict extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        DictService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, },
                            ...
                            { title: '顺序号',width: 100,dataIndex: 'sort',key: 'sort', sorter: true,},
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='DICT_EDIT'>
                            ...
                            <Authority authorityId='DICT_DELETE'>
				...
				okText='提交' cancelText='关闭'>
                    <DictAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
				...
				okText='提交' cancelText='关闭'>
                    <DictEdit ref={this.onEditRef} dict={this.state.dict} handEditOKFinish ={this.handEditOKFinish} />
                </Modal>
            </div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:(await DictService.getToken()),
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            dict:await DictService.getDict(this.state.selectedRowKeys[0]),
        })
    }
    handleEditOK = () =>{       
        this.child.handleOK();
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await DictService.delete(selectedRowKeys)
