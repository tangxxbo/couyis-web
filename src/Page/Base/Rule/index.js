...
class Rule extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        RuleService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, },
                            ...
                            { title: '当前', width: 120, dataIndex: 'counts', key: 'counts', sorter: true, },
                            { title: '格式', width: 120, dataIndex: 'dateFormat', key: 'dateFormat',...this.getSearchProps("dateFormat") },
                            { title: '流水位', width: 120, dataIndex: 'number', key: 'number', sorter: true, },
			                { title: '前缀', width: 120, dataIndex: 'prefix', key: 'prefix', sorter: true, ...this.getSearchProps("prefix")},
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='RULE_EDIT'>
                            ...
                            <Authority authorityId='RULE_DELETE'>
				...
				<Table
					...
					rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加规则' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                    <RuleAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
                <Modal title='修改规则' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <RuleEdit ref={this.onEditRef} rule={this.state.rule} handEditOKFinish ={this.handEditOKFinish} />
                </Modal>
			</div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await RuleService.getToken()
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            rule:await RuleService.getRule(this.state.selectedRowKeys[0])
        })
    }
	...
	handleDelete = () => {
		...
		confirm({
			...
			async onOk() {
                await RuleService.delete(selectedRowKeys)
