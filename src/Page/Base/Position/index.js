...
class Position extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        PositionService.pageList({...pagination, ...filters, ...sorter}).then(res => {
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='POSITION_EDIT'>
                            ...
                            <Authority authorityId='POSITION_DELETE'>
				...
				<Table
					...
					rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加职位' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                    <PositionAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish} />
                </Modal>
                <Modal title='修改职位' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <PositionEdit ref={this.onEditRef} position={this.state.position} handEditOKFinish ={this.handEditOKFinish}/>
                </Modal>
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await PositionService.getToken()
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            position:await PositionService.getPosition(this.state.selectedRowKeys[0])
        })
    }
    handleEditOK = () =>{       
        this.child.handleOK();
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await PositionService.delete(selectedRowKeys)
