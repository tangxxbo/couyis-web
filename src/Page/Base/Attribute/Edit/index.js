...
class Edit extends Component {
	...
	componentWillMount =async ()=>{
		const dictTree = await DictService.getTreeDict()
		this.setState({
			dictTree
		})	
	}
    ...
    onFinish = async (values) =>{
		...
		await AttributeService.edit(values);
	...
	componentDidMount= () =>{
		const { attribute } = this.props;
			if(attribute.dataType !=null){
				this.setState({
					inputUi:this.inputUi(attribute.dataType)
				})
			}
	...
	componentWillReceiveProps =(nextProps)=>{
		if(this.props.attribute !== nextProps.attribute){
			if(nextProps.attribute.dataType !=null){
				this.setState({
					inputUi:this.inputUi(nextProps.attribute.dataType)
				})
			}
			if(nextProps.attribute.attributeValues !=null){
				this.setState({
					attributeValues:nextProps.attribute.attributeValues
				})
			}
		}
	}
	render() {       
		...
		return (
				...
				<Form {...formItemLayout}  ref={this.formRef}  onFinish={this.onFinish} initialValues={{...attribute}}>
					<Tabs >
