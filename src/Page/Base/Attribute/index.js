...
class Attribute extends Component {
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        AttributeService.pageList({...pagination, ...filters, ...sorter}).then(res => {
    ...
    render() {
        const columns = [   { title: '序号', width: 80, render: (text, record, index) => `${index + 1}`, },
                            ...
                            { title: '数据类型', dataIndex: 'dataType', key: 'dataType', sorter: true, width: 250 ,
                            filters: [
                                { text: '字符', value: 'CHAR' },
                                { text: '数字', value: 'NUMBER' },
                                { text: '日期', value: 'DATE' },
                                { text: '数据字典', value: 'DCIT' },
                              ],
                            render: ((data) => {
                                    switch (data) {
                                    case 'CHAR':
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                        <Authority authorityId='ATTRIBUTE_EDIT'>
                        ...
                        <Authority authorityId='ATTRIBUTE_DELETE'>
				...
				<Table
					...
					rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加属性' visible={this.state.addVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleAddOK} onCancel={this.handleAddCancel}
				okText='提交' cancelText='关闭'>
                    <AttributeAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish}/>
                </Modal>
                <Modal title='修改属性' visible={this.state.editVisible} destroyOnClose={true} centered width={800}
				onOk={this.handleEditOK} onCancel={this.handleEditCancel}
				okText='提交' cancelText='关闭'>
                    <AttributeEdit ref={this.onEditRef} attribute={this.state.attribute} handEditOKFinish ={this.handEditOKFinish} />
                </Modal>
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await AttributeService.getToken(),           
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            attribute:await AttributeService.getAttribute(this.state.selectedRowKeys[0]),
        })
    }
	...
	handleDelete = () => {
		...
		confirm({
			...
			async onOk() {
                await AttributeService.delete(selectedRowKeys)
