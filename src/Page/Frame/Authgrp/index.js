...
import AuthgrpAdd from './Add'
import AuthgrpEdit from './Edit'
const {confirm} = Modal
class Authgrp extends Component {
    state = {
        dataSource: [],
        pagination:{},
        filters:{},
    ...
    search = (pagination=this.state.pagination, filters=this.state.filters, sorter=this.state.sorter) => {
        ...
        AuthgrpService.pageList({...pagination, ...filters, ...sorter}).then(res => {
            ...
            <div className="table-list"> 
               <div className='table-list-top'>
                    <div className='table-list-top-tp'>
                            <Authority authorityId='AUTHORITY_GROUP_EDIT'>
                            ...
                            <Authority authorityId='AUTHORITY_GROUP_DELETE'>
				...
				<Table
					...
					rowKey={(record) => `${record.id}`}
					 />
                <Modal title='添加权限组' visible={this.state.addVisible} destroyOnClose={true} centered width={800} onOk={this.handleAddOK} onCancel={this.handleAddCancel} okText='提交' cancelText='关闭'>
                    <AuthgrpAdd ref={this.onAddRef} token = {this.state.token} handAddOKFinish={this.handAddOKFinish}/>
                </Modal>
                <Modal title='修改权限组' visible={this.state.editVisible} destroyOnClose={true} centered width={800} onOk={this.handleEditOK} onCancel={this.handleEditCancel} okText='提交' cancelText='关闭'>
                    <AuthgrpEdit ref={this.onAddRef} authgrp={this.state.authgrp} handEditOKFinish={this.handEditOKFinish}/>
                </Modal>
			</div >
        )
    }
	...
	handleAdd = async () => {
		this.setState({
            addVisible: true,
            token:await AuthgrpService.getToken()
		})
    }
    ...
    handleEdit = async () => {
        ...
        this.setState({ 
            editVisible: true, 
            authgrp:await AuthgrpService.getAuthgrp(this.state.selectedRowKeys[0])
        })
    }
	...
	handleDelete = (id) => {
		...
		confirm({
			...
			async onOk() {
                await AuthgrpService.delete(selectedRowKeys)
